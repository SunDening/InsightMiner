  ...
  <div class="auth-page">
    ...
    <div class="auth-card">
      ...
      <div class="auth-header">
        ...
        <h1>登录</h1>
        <p class="auth-subtitle">欢迎回到 InsightMiner</p>
      </div>

      <!-- Tab: 密码登录 / 验证码登录 -->
      <div class="auth-tabs">
        <button
          :class="['tab-btn', { active: tab === 'password' }]"
          @click="tab = 'password'">
          密码登录
        </button>
        <button
          :class="['tab-btn', { active: tab === 'code' }]"
          @click="tab = 'code'">
          验证码登录
        </button>
      </div>
      ...
      <form v-else class="auth-form" @submit.prevent="handleCodeLogin">
        <div class="field">
          ...
          <div class="input-wrap">
            <span class="input-icon">✉</span>
            <input v-model="codeForm.email" type="email" placeholder="your@email.com" required />
          </div>
        </div>
        <div class="field">
          <label>验证码</label>
          <div class="code-row">
            <div class="input-wrap code-input-wrap">
              <span class="input-icon">✉</span>
              <input v-model="codeForm.code" type="text" placeholder="6 位数字" maxlength="6" required />
            </div>
            <button type="button" class="send-code-btn" :disabled="codeSending || codeCountdown > 0" @click="handleSendCode">
...
export default {
  ...
  setup() {
    ...
    async function handleSendCode() {
      ...
      try {
        await sendCode(codeForm.value.email)
    ...
    async function handleCodeLogin() {
      ...
      loading.value = true
      try {
        await auth.doLoginWithCode(codeForm.value)
        router.push('/')
      } catch (e) {
        error.value = e?.response?.data?.detail || '登录失败'
      } finally {
